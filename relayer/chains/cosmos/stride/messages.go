...
func (msg MsgSubmitQueryResponse) ValidateBasic() error {
	...
	if err != nil {
		return errors.Wrapf(sdkerrors.ErrInvalidAddress, "invalid fromAddress in ICQ response (%s)", err)
	}
	// check chain_id is not empty
	if msg.ChainId == "" {
		return errors.Wrap(sdkerrors.ErrInvalidRequest, "chain_id cannot be empty in ICQ response")
	}
